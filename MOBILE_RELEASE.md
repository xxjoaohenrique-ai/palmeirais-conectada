...
Para gerar um Digital Asset Links **sem valores fictícios**, configure a variável `APP_SIGNING_FINGERPRINTS` com o SHA-256 da assinatura efetiva (pode conter mais de uma impressão digital separada por vírgula) e execute `node scripts/generate-assetlinks.mjs`. Revise e publique o arquivo gerado em `.well-known/assetlinks.json` somente depois que os certificados e o pacote Android estiverem confirmados. Essa impressão digital é informação pública; o keystore e as senhas continuam privados.
